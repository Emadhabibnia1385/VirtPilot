...
REPO="https://github.com/Emadhabibnia1385/VirtPilot.git"
DIR="/opt/virtpilot"
SERVICE="virtpilot"
...
header() {
  ...
  echo -e "${C}║${N}  ${B}${M}  ╚═══╝  ╚═╝╚═╝  ╚═╝   ╚═╝   ╚═╝     ╚═╝╚══════╝ ╚═════╝    ╚═╝   ${N}  ${C}║${N}"
  echo -e "${C}║${N}                                                                        ${C}║${N}"
  echo -e "${C}║${N}              ${B}🚀 VirtPilot - Virtualizor Telegram Manager${N}              ${C}║${N}"
  echo -e "${C}║${N}                 ${B}Repo:${N} github.com/Emadhabibnia1385/VirtPilot             ${C}║${N}"
  echo -e "${C}║${N}                                                                        ${C}║${N}"
  echo -e "${C}╚════════════════════════════════════════════════════════════════════════╝${N}"
  echo ""
}
...
clone_or_update_repo() {
  ...
  if [[ -d "$DIR/.git" ]]; then
    ...
    cd "$DIR"
    git fetch --all --prune
    # اگر branch شما main نیست، این خط را عوض کن
...
configure_env() {
  echo ""
  info "VirtPilot Configuration"
  read -r -p "Enter your Telegram Bot TOKEN: " BOT_TOKEN
  [[ -n "${BOT_TOKEN// }" ]] || err "BOT_TOKEN cannot be empty"

  read -r -p "Check interval seconds [default: 300]: " INTERVAL
  INTERVAL="${INTERVAL:-300}"
  [[ "$INTERVAL" =~ ^[0-9]+$ ]] || err "CHECK_INTERVAL_SECONDS must be numeric"
...
[Unit]
Description=VirtPilot Telegram Bot
...
start_service() {
  ...
  ok "VirtPilot installed successfully!"
  echo ""
...
remove_bot() {
  ensure_safe_cwd
  read -r -p "Are you sure you want to remove VirtPilot? (yes/no): " confirm
  if [[ "$confirm" != "yes" ]]; then
    info "Cancelled"
    return
  fi
  ...
  systemctl daemon-reload
  rm -rf "$DIR"
  ok "VirtPilot removed completely"
}
